        ...
        <div className="w-full pt-[40vh] pb-[10vh]">
          <div className="flex flex-col items-end gap-10">
            {features.map((feature) => (
              <div
                key={feature.id}
                className="aspect-square w-[30rem] p-6 bg-gradient-to-b from-[#080808]/0 to-[#080808]/40 border-2 border-white/10 rounded-[15px] flex flex-col justify-center items-center text-center shadow-card relative gap-2"
              >
                {/* number */}
                <h1 className="text-9xl font-enoch text-neutral-900/50">
                  {feature.id.toString().padStart(2, "0")}
                </h1>
                <h2 className="font-bold text-[#B0B3B8] text-3xl mt-2">
                  {feature.title}
                </h2>
                <p className="text-[#A1A5AB] text-2xl mt-4">
                  {feature.description}
                </p>
              </div>
            ))}
